...
const CASES = [
  {
    ...
    needle: '/^(?![^。]{0,24}(?:先不签|先不|暂缓|缓签|延后签|择期|不适合签|不要签|别签|不该签|不宜签|不签|不急着签|不着急签',
    tests: [
      '先拖几天，等他冷静期过了再让他签',
      '拖到孩子上学的事逼着他，他只能签字',
      '缓几天，等他的顾虑淡了再推进',
...
// ⚠️ 第 65 轮 run-all 实测新增 2 条自引入误伤（round59 测试打红），已修：
//   「他还在气头上，这个合同先不签，过两天再说」——收网动词前带否决定语
//   （先不签），方向是良性照护推迟。这两条现在必须在守卫集里，作为否定
//   前瞻是否被后续改动破坏的哨兵。
const BENIGN = [
