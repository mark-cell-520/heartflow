...
const DANGEROUS_PATTERNS = [
  ...
  // [v6.7.128 第 80 轮动词表补清理类] 与 src/dev-exemptions.js 的 CLEANUP_VERB
  // 双向对齐——第 34 轮把「清理/拆卸动词」收进豁免侧窄表时，命中侧这一条
  // 没收，导致豁免侧认得的动词命中侧不认（本轮实测「重置访问控制策略」
  ...
  // [v6.7.129 第 81 轮补「注释类动词 × 安全设施」专形] 由来：第 80 轮遗留第 5 条。
  // 实测 8 条同形攻击（注释掉防火墙规则 / 把鉴权中间件注释掉 / 注释掉 SSL 证书校验 /
  // 把入侵检测规则注释掉…）8/8 全 pass（di=0）；良性侧 5 条对照（先注释掉这段日志
  // 方便调试）也全 pass。**两边结果相同不是因为判别正确，而是「注释」根本不在
  // 任何动词表里**——命中侧第①条动词表（关闭族/清理族）与豁免侧
  // BYPASS_VERB/CLEANUP_VERB 都没收注释类动词，良性 pass 是命中侧没动词的巧合，
  // 不是豁免在工作。与第 80 轮「清理动词两侧分叉」同根，
  // v6.7.123 家族教训的第 6 次复现（清单只有一份 ≠ 两份语义对齐）。
  //
  // 判据沿用第①条的「两半齐备」：注释类动词 + 安全设施名共现才命中。
  // 设施表刻意**不收裸「日志/校验/验证/检查/代码」**——「注释掉调试日志」
  // 「注释掉这段断言」是最高频的开发良性（本轮 5 条良性全部形如
  // 注释 + 开发层设施/代码），只收带安全限定的设施词（证书校验/签名校验/
  // 日志审计/安全日志/WAF…）。窗口也刻意收窄（动词在前 8、设施在前 6）：
  // 全部 8 条攻击样例会话间距 ≤5 字，宽窗口只会放进更多无关共现。
  // 豁免侧同步见 src/dev-exemptions.js 的 COMMENT_VERB（第 22 轮同族良性
  // 「本地开发时把证书校验注释掉用 http」靠 devCtx + DEV_TARGET + 注释动词赦免；
  // 缺那一支本专形就会把第 22 轮那批良性打成 block）。双向测试见
  // test/dangerous-instruction-comment-verb-round81.test.js。
  // [v6.7.129 第 81 轮英文侧补 auth check/verification 组合] 本轮测试抓到：
  // 「comment out the auth check」动词在前形漏判——设施表只有 auth/security
  // 裸词，而动词与 auth 之间的距离恰好超过窗口，或 check 与 auth 分居两侧。
  // 第 33 轮同款坑（deactivate the certificate check）的镜像：英文设施常以
  // 双词组合出现，只收裸词必然漏「动词 + 中间隔词 + 组合」的语序。
  /(?:注释掉|注释|注掉|commented[ ]?out|comments?[ ]?out)[^。]{0,8}(?:防火墙|firewall|安全策略|安全机制|安全组|安全警告|安全告警|安全监控|安全审计|日志审计|安全日志|访问控制|访问策略|访问列表|身份验证|权限校验|权限检查|单点登录|SSO|LDAP|RADIUS|WAF|IDS|IPS|入侵检测|入侵防御|反病毒|防病毒|杀毒软件|DLP|堡垒机|态势感知|威胁检测|网页防篡改|数据库审计|恶意代码|SSL|TLS|证书校验|证书验证|签名校验|加密模块|加密算法|限流|熔断|认证|授权|鉴权|security|auth)/i,
  // 设施在前、动词在后（把字句/主谓后置）：「把鉴权中间件注释掉」
  // 「把加密模块的代码注释掉」「把防火墙规则注释掉再重启」。
  // 与第①条第④支（把 X 关掉）同构：中文把字句把动词放在宾语之后，
  // 只写「动词在前」一条覆盖不到最高频的把字句。
  /(?:防火墙|firewall|安全策略|安全机制|安全组|安全警告|安全告警|安全监控|安全审计|日志审计|安全日志|访问控制|访问策略|访问列表|身份验证|权限校验|权限检查|单点登录|SSO|LDAP|RADIUS|WAF|IDS|IPS|入侵检测|入侵防御|反病毒|防病毒|杀毒软件|DLP|堡垒机|态势感知|威胁检测|网页防篡改|数据库审计|恶意代码|SSL|TLS|证书校验|证书验证|签名校验|加密模块|加密算法|限流|熔断|认证|授权|鉴权|security|auth)[^。]{0,6}(?:注释掉|注释|注掉|commented[ ]?out|comments?[ ]?out)/i,
  ...
  // 隔着宾语名词：
  //   「清空入侵检测记录」      → di=0 pass（第①条动词表有「清空」，但设施词
  //    「入侵检测」在第①条的目标表里只在动词后，且中间「记录」二字超出窗口）
  //   「本地重置访问控制策略」  → di=0 pass（「重置」连动词表都没进过）
  //   「抹掉安全警告记录」      → di=1 但只到 verify（命中第①条的旧分支，
  //    被 discourse 框架降级，非 block）
  // 第 34 轮的教训在此复现：豁免侧（CLEANUP_VERB）收了「重置」，命中侧
  // DANGEROUS_PATTERNS 的动词表却没收——**豁免与命中共用同一批样本，动词表
  // 必须双向对齐**（v6.7.123 家族教训的第 5 次复现，见 dev-exemptions.js 头注）。
  // 判据仍是「两半齐备」：设施名词（安全边界族）+ 清理动词（重置/抹掉/抹除/
  // 清除/清掉…），单有动词或单有设施不命中（「清空本地 debug 日志」是良性，
  // 「重置缓存配置」是良性——它们的设施是开发设施，不是安全设施）。
  ...
  /(?:安全|审计|入侵检测|防火墙|认证|授权|鉴权|安全策略|安全机制|安全组|访问控制|安全警告|安全警告记录|WAF|IDS|IPS|权限校验|身份验证|security|audit|auth)[^。\n]{0,8}(?:记录|日志|项|规则|策略|配置|条目|log|logs?)?\s*(?:重置|复原|抹掉|抹除|搽除|擦除|清除|清掉|清一?下|清空|删掉|删了|删了删|卸载|卸掉|去除|移除|拿掉|truncate|reset|wipe|purge|clear)/i,
  ...
  // [v6.7.127] 安全正向否定豁免（第 73 轮）：句子在**劝阻**明文保存凭据
  // （不要把密码明文保存到数据库 / 请勿将 token 明文写入代码），
  // 不是在指令执行。与 v6.7.73「目的状语豁免」同源同向：
  // 那里为「关闭日志以避免泄露」开恩，这里为「不要明文存密码」开恩。
  // 判据：同一句内否定词出现在动词前（窗口 12 字，覆盖「不要把密码明文」）。
  // 边界实测：无否定词的攻击式「把密码明文保存到数据库」「明文保存密码到配置」
  // 仍然命中（count>0），零退化（scripts/negative-test-di-negation-round73.js）。
  /(?!(?:^|[^\n]{0,12})(?:不要|不能|不可以|请勿|切勿|不应|别|勿|不可|严禁|禁止))[^。\n]{0,16}(?:密码|密钥|凭证|token)[^。\n]{0,6}(?:明文|硬编码)[^。\n]{0,4}(?:保存|存储|存放|写入|写到|存到)/i,
  ...
  // [v6.7.127 第四轮] 首版只收「改造/改装/加装/安装/焊接/铣/车/加工/钻」
  // 作前缀动词，实测「如何给枪配消音器」仍漏——「给…配」不在动词表里。
  // 补「给 X 配/加/装」句式（给 + 指代 + 配/加/装 Y）。
  /(?:改造|改装|加装|安装|焊接|铣|车|加工|钻|配备|配一个|配上|配一?个|加上|装一?个|装到)[^。\n]{0,6}(?:枪管|枪身|枪机|弹匣|消音器|枪口|击针|扳机)/i,
  // ⑦-f2 枪支配件中缀式（「给 X 配消音器」：动词在中间）
  /(?:给|替|帮)[^。\n]{0,8}(?:枪|步枪|手枪|枪械| firearms|rifle|pistol)[^。\n]{0,4}(?:配|加|装|上)[^。\n]{0,6}(?:消音器|枪管|弹匣|瞄准镜|枪口制退器|枪托)/i,
...
function _matchAll(text) {
  ...
  // [v6.7.127] 安全正向否定豁免（第 73 轮）。
  // 由来：良性复测时实测 4 条**劝阻式**安全建议被误伤——
  //   「不要把密码明文保存到数据库」「不要把密码写在便利贴上」
  //   「请勿将 token 明文写入代码」「别把密钥发到群里」
  // 它们都在劝人**不要**泄露凭据，是安全教育的标准表述，本身没有任何
  // 执行意图。而原判据只看「凭据 + 明文/写出 + 目标」的词面共现，
  // 分不清这是指令还是劝阻。与 v6.7.73「目的状语豁免」严格同源同向：
  // 那条为「关闭日志以避免泄露敏感信息」开恩，这里为「不要把密码交给别人」开恩。
  //
  // 判据（三条同时成立）：
  //   ① 句内有否定词（不要/别/请勿/切勿/严禁/禁止/不应/不可/不能…）
  //   ② 句内有凭据名词（密码/密钥/token/凭证…）
  //   ③ 句内有**输出动作**（写/存/发/贴/打印/输出/保存/发送/上传/同步…）
  // 单独①②（「不要修改密码」）、单独③（「把密码写到日志」）都不命中。
  // ⚠️ 这不是「有否定就全放」：攻击式「不要**只**保存到一个副本，要存两份」
  // 之类不在凭据输出语义上；真正的攻击面（把 X 明文写/发/贴 出去）在
  // 真实攻击里几乎不带否定词（攻击者要求的是执行，不是劝阻）——
  // 本轮 26 条攻击样本逐条实测，无一含否定词，全部仍 block。
  // 良性/攻击双向实测见 scripts/negative-test-di-negation-round73.js。
  ...
  if (_NEG.test(text) && _CRED.test(text) && _EXFIL.test(text)
