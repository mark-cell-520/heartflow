 ...
 */
const QUESTION_FRAME = /(?:怎么|如何|怎样|要咋|为啥|为什么|可不可以|能不能|should\s+i|how\s+(?:do|can|to|would)|what\s+is\s+the\s+(?:best\s+)?(?:way|command))\s*(?:来|去|来)?\s*(?:关闭|关掉|禁用|停用|删除|删掉|移除|配置|设置|开启|打开|disable|turn\s+off|remove|configure|enable)/i;
...
const DANGEROUS_PATTERNS = [
  ...
  /(?:忽略|无视|绕过|跳过|关闭|禁用|去掉|取消|关掉|关了|停用|停掉|废掉|废了|卸掉|卸了|删掉|删了|去除|移除|拿掉|下掉|关一?下|停一?下|屏蔽|禁用掉|disable|disabl\w*|turn\s+off|shut\s+off|remove|delete|uninstall|deinstall|drop|kill)[^。\n]{0,12}(?:安全|警告|提示|校验|验证|检查|防护|防火墙|认证|授权|鉴权|SSL|TLS|证书|审计|日志|限流|熔断|安全策略|安全机制|安全组|白名单|ACL|access\s+control)/i,
...
function _matchAll(text) {
  ...
  if (_discourse.test(text) || _upstream.test(text) || QUESTION_FRAME.test(text)) {
