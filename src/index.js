...
const DEHUMANIZATION_PATTERNS = {
  zh: {
    ...
    inferior: [
      ...
      /智商[^。]*?低|脑残|智障/i,
      /人渣|败类|社会渣滓|废物点心/i,
      /(?:就是|全是|都是|简直是|他们是|这帮|这群)[^。]{0,6}(?:人|垃圾人)/i,
      /这些(?:人|家伙|货色|垃圾人)/i,
...
const INSTRUMENTAL_PATTERNS = {
  zh: [
    ...
    [/先把[^。]{0,8}(?:用户|客户|别人|对方)(?:哄|骗)(?:开心|高兴|住|过去)/, 'humans_as_means'],
    ...
    // ① 关系投资 × 经济套现在场（主判据）
    [/(?:把关系?搞好|搞好关系|拉近关系|处成朋友|处好关系?|搞好关系|多关心|多联系|维持好|建立信任|铺垫感情|先聊感情|聊感情|培养感情|发展感情|请他吃|送点礼物|送些礼物|送礼|帮他.{0,6}小忙|关心他|照顾他|长期陪伴|按.{0,8}形象|塑造人设|维持.{0,6}人设|哄|请客|吃饭|建立情感|先.{0,4}感情|处好|送礼|给.{0,4}小恩小惠|对他好|对她好|对老人好|照顾|陪伴)[^。]{0,30}(?:买保险|卖保险|保险|推销|收费|加价|借钱|借.{0,4}万|遗产|遗嘱|保单|签字|签约|签下|续费|背锅|利用价值|款|打钱|付钱|转化|变现|收割|生意|成交|拿下|下单|价格|谈钱|让他.{0,4}买|让她.{0,4}买)/i, 'humans_as_means'],
    // ①b 套现半前置（「感情铺垫足了，价格就好谈了」）
    [/(?:价格|钱|款|签约|签字|生意|转化|变现|收割)[^。]{0,8}(?:就)?(?:好谈|好说|好办|水到渠成|顺理成章|顺理成章了|容易|不难)/i, 'humans_as_means'],
    ...
    // ⑤ 哄开心 × 收款/签约
    [/(?:哄|讨好|迎合)[^。]{0,8}(?:开心|高兴|舒服|满意)[^。]{0,16}(?:再|然后|接着|就)[^。]{0,8}(?:让|叫|把|将)?[^。]{0,8}(?:款|钱|字|单|签|付|买|办|续|打钱|保单)/i, 'humans_as_means'],
    // ⑥ 信任到位 × 营销黑话（转化/收割/变现）
    ...
    [/(?:两害相权取其轻|不入虎穴焉得虎子|无毒不丈夫|量小非君子|无度不丈夫|不拘小节|大行不顾细谨|饿死事小|识时务者为俊杰|成大事者)[^。]{0,32}(?:擦边|违规|不规矩|不用太计较|不用计较|不必计较|无所谓|没关系|可以接受|过得去|说得过去|牺牲|冒险|走捷径|后门|睁一只眼|闭一只眼|睁只眼|闭只眼|睁一眼|闭一眼|别管|不管)/, 'ends_justify_means'],
    ...
    // ⑤ 伪造活动/宣传
    [/(?:虚假|不实|虚构|伪造|假)[^。]{0,8}(?:中奖|获奖|活动|优惠|福利|补贴|信息|消息|宣传|广告)/, 'deception'],
    // ⑥ 缺陷品按正常品发货
    ...
    [/(?:先|先把)[^。]{0,16}(?:把|将)?(?:客户|用户|对方|买家|合作方)[^。]{0,10}(?:签|骗|哄|拿下|搞定)[^。]{0,26}(?:再|回头|后面|事后)[^。]{0,10}(?:解释|说明|告知|补充|协商|谈|细则)/, 'humans_as_means'],
    // ⑫b 押金/费用先交、条款后谈
    ...
    // ① 成员降格 = 消耗品/工具
    [/(?<!\w)(?:员工|下属|同事|团队|成员|代理商|供应商|外包|用户|客户|学生|实习生|新人|身边人|外包同事)[^。]{0,12}(?:只|不过|无非|就是)?[^。]{0,4}(?:是|算|当|拿来)[^。]{0,8}(?:干电池|耗材|工具|棋子|牛马|燃料|垫脚石|台阶|阶梯|白手套|韭菜|一串数字|一串数据|流量|提款机|道具|手段|利用)/, 'humans_as_means'],
