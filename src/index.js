...
const DEHUMANIZATION_PATTERNS = {
  zh: {
    ...
    object: [
      ...
      /(?:把|将|当|当成|视为|当作|看作)[^。]{0,6}(?:人|他们|她们|员工|工人|士兵|学生|孩子|群众|百姓|用户|客户)[^。]{0,10}(?:牲口|畜生|禽兽|猪狗|牛马)/i,
    ...
    dehumanization_frames_zh_dispose: [
      /(?:根本|压根|就|并)?(?:不|没|并非|并不|根本不|压根不)(?:是|算|属于)(?:个|一名|一个)?(?:人|人类|人类了)[^。]{0,14}(?:[^。]{0,4}(?:只是|仅仅是|不过是))?[^。]{0,20}(?:工具|机器|零件|物件|东西|货物|牲口|数字|编号|干活的|说话的|处理掉|清除|消灭|干掉)/i,
      // ④-b 否认人属 + 修饰词 + 物名词（「并非人类，只是干活机器」）
      /(?:根本|压根|就|并)?(?:不|没|并非|并不|根本不|压根不)(?:是|算|属于)(?:个|一名|一个)?(?:人|人类|人类了)[^。，。]{0,14}(?:，|,)?[^。]{0,4}(?:只是|仅仅是|不过是)[^。]{0,6}(?:会说话|会干活|能干活|干活|说话|移动|计数|执行|服从|消耗)[^。]{0,8}(?:的)?(?:工具|机器|零件|物件|东西|货物|牲口|数字|编号)/i,
    ],
  },
  en: {
    animal: [
