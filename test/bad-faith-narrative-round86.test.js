...
// 本轮未覆盖的 7 条（形状已解剖、候选判据在试错台 v5，未达收敛标准）
const UNTILED = [
  '别和他辩了，他根本不是在讨论问题',
  '只要你不接招，他就说你默认了',
  '这套立体防御就是为了让他永远正确',
...
t(`攻击样本命中 bad_faith 维度 ≥ 17/24（实测 17；未覆盖 7 条见 UNTILED）`, () => {
...
t('未覆盖的 7 条不得被 bad_faith 命中（允许 pass，不允许假命中）', () => {
  const bad = UNTILED.filter(s => (gate.gate(s).findings || []).some(f => f.dimension === 'bad_faith'));
  assert.deepStrictEqual(bad, [], `意外命中: ${bad.join(' | ')}`);
});
