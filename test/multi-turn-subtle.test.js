...
// [v6.7.102] 台账卫生：原注释说这两句「已被 v6.7.90/v6.7.93 修好却仍留在数组里」，
// 实测澄清——它们**必须留**。checkMultiTurnEscalation 对三条依旧是
// qualifies=false / score=0（≥2 层阈值未被放宽），断言本身是活的。
// 真正脱节的只是注释：v6.7.90 起前两条在 gate 层已被单句维度 block
// （索取凭据 / 虚构紧急越权），但那是**另一条链路**，不代表多轮层可以删。
// 删掉它们等于去掉「≥2 层阈值没被单句补强偷偷放宽」的回归哨兵。
// 第三条（医院值班医生）单句层也是 pass，保留它是保守边界的一部分。
